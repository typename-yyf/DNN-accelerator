    ...
    *,
    ...
    for (device_params, device_grads, device_exp_avgs, device_exp_avg_sqs,
        ...
        for param in device_params:
            if ratio.get(param):
                ratio_list.append(ratio.get(param))
            else:
        ...
        if capturable:
            ...
            ratio_list = _stack_if_compiling(ratio_list)
            ...
            torch._foreach_addcdiv_(device_params, device_exp_avgs, denom, ratio_list)
        else:
            bias_correction1 = [1 - beta1 ** _get_value(step) for step in device_state_steps]
            bias_correction2 = [1 - beta2 ** _get_value(step) for step in device_state_steps]

            step_size = _stack_if_compiling([(lr * r / bc) * -1 for bc, r in zip(bias_correction1, ratio_list)])
